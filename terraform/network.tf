...
resource "azurerm_subnet" "aks_argocd" {
  ...
  address_prefixes     = [var.subnet_prefixes_aks_argocd]
}
...
resource "azurerm_subnet" "appgw_argocd" {
  ...
  resource_group_name  = azurerm_resource_group.resource_group.name
  virtual_network_name = azurerm_virtual_network.vnet.name
  address_prefixes     = [var.subnet_prefixes_appgw_argocd]
}
...
resource "azurerm_subnet" "aks_webapp" {
  ...
  resource_group_name  = azurerm_resource_group.resource_group.name
  virtual_network_name = azurerm_virtual_network.vnet.name
  address_prefixes     = [var.subnet_prefixes_aks_webapp]
}
...
resource "azurerm_subnet" "appgw_webapp" {
  ...
  resource_group_name  = azurerm_resource_group.resource_group.name
  virtual_network_name = azurerm_virtual_network.vnet.name
  address_prefixes     = [var.subnet_prefixes_appgw_webapp]
}
